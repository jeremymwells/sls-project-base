...
const runActions = {
  web: async (argv) => {
    ...
    const startResultCode = await helpers.runProcess(`${envVars} concurrently --kill-others "node localProxy.js" "expo start:web --localhost"`);
...
module.exports.build = async (argv) => {
  const envVars = await getWebEnvVars(argv);
  const buildResultCode = await helpers.runProcess(`${envVars} expo build:web`);
